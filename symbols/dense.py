...
def DenseNet(units, num_stage, growth_rate, num_class, reduction=0.5, drop_out=0., bottle_neck=True, bn_mom=0.9, workspace=512):
    ...
    body = mx.sym.Convolution(data=data, num_filter=growth_rate*2, kernel=(7, 7), stride=(1,1), pad=(1, 1),
                              no_bias=True, name="conv0", workspace=workspace)
    body = mx.sym.BatchNorm(data=body, fix_gamma=False, eps=2e-5, momentum=bn_mom, name='bn0')
    body = mx.sym.Activation(data=body, act_type='relu', name='relu0')
    body = mx.symbol.Pooling(data=body, kernel=(2, 2), stride=(2,2), pad=(1,1), pool_type='max')
