...
net=import_module('symbols.'+"kvgg")
...
train=wkiter("root",istrain=1,fstart=0,friend=15,batch_size=batch_size,end=.1)
test=wkiter("root",fstart=15,friend=5,batch_size=batch_size*10,end=.1)
print ("train",train.totalnum(),"eval",test.totalnum())
for i in range(epochs):
	print("epoch",i)
        checkpoint=keras.callbacks.ModelCheckpoint(filepath='save/fullydijetsame_'+str(i),monitor='val_loss',verbose=0,save_best_only=False,mode='auto')
